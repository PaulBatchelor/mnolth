...
INTERFACE INLINE void * cptr_value(pointer p) { return (void *)string_value(p); }
...
struct scheme_interface * scheme_get_interface(scheme *sc)
{
    return sc->vptr;
}
#endif
...
void* scheme_get_external_data(scheme *sc) {
 return sc->ext_data;
}
...
/* this zeros out a port... bugfix by paul */
static void clear_port(port *p)
{
    size_t i;
    size_t size;
    unsigned char *mem;
...
/* just like load_string, except the interactive_repl flag isn't set */
void scheme_eval_string(scheme *sc, const char *cmd) {
  dump_stack_reset(sc);
  sc->envir = sc->global_env;
  sc->file_i=0;
  clear_port(&sc->load_stack[0]);
...
pointer scheme_panic(scheme *sc, const char *msg)
{
    Error_0(sc, msg);
}
