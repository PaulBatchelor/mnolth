...
/** Return a single sample. This is something I added for patchwerk.
 * This will discard panning data.
 */

float xm_compute_sample(xm_context_t *);

/** Tick function
*/

void xm_please_tick(xm_context_t *);


/** Returns sample from single channel.
 * Be mindful of state. Used in patchwerk.
 */

float xm_compute_channel(xm_context_t *, int chan);

/** Ticks a channel
 * Sort of a hack, but useful if you want to process individual channels
 */

void xm_please_tick_channel(xm_context_t *, int chan);
#endif
