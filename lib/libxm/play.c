...
static void xm_handle_global_effects(xm_context_t*, xm_channel_context_t*, xm_pattern_slot_t*);
...
static void xm_row_channel(xm_context_t*, int chan);
static void xm_tick(xm_context_t*);
static void xm_tick_channel(xm_context_t*, int chan);
...
/** Process only the global effects on a row.
 * Needed by xm_row_channel.
 */

static void xm_handle_global_effects(xm_context_t* ctx, xm_channel_context_t* ch, xm_pattern_slot_t* s)
{
...
/** hacked up version of xm_rowthat only works on specific channel
 * designed to be used with use with xm_state.
 */

static void xm_row_channel(xm_context_t* ctx, int chan)
{
    int i;
    xm_channel_context_t* ch;
    ch = &ctx->channels[chan];
    ...
    xm_pattern_slot_t* s = cur->slots + ctx->current_row * ctx->module.num_channels + chan;
    ...
    /* global effects for all channels need to be processed still */
    for(i = 0; i < ctx->module.num_channels; ++i) {
        if(i != chan) {
            xm_pattern_slot_t* s = cur->slots + ctx->current_row * ctx->module.num_channels + i;
            ch = ctx->channels + i;
            if(s->effect_type != 0xE || s->effect_param >> 4 != 0xD) {
                xm_handle_global_effects(ctx, ch, s);
            }
        }
    }
...
/** hacked up version of xm_tick that only works on specific channel
 * designed to be used with use with xm_state.
 */
static void xm_tick_channel(xm_context_t* ctx, int chan)
{
...
float xm_compute_sample(xm_context_t *ctx)
{
    uint8_t i;
    float fval;
    float fgvol;
    ...
    for(i = 0; i < ctx->module.num_channels; ++i) {
        ...
        if(!ch->muted && !ch->instrument->muted && i == 1) {
            out += fval * ch->actual_volume * fgvol;
        }
...
void xm_please_tick(xm_context_t* ctx)
{
    xm_tick(ctx);
}

void xm_please_tick_channel(xm_context_t* ctx, int chan)
{
    xm_tick_channel(ctx, chan);
}

float xm_compute_channel(xm_context_t *ctx, int chan)
{
    xm_channel_context_t* ch;
    float fval;
    float out;
